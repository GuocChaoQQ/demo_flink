 ...
 **/
class ResultDwbKuduSink extends RichSinkFunction[String]{
  var kuduClient: KuduClient = _
  var kuduSession: KuduSession = _
  var dwsMemberTable: KuduTable = _


  def inserintoDwsMember(jsonObject: JSONObject): Unit = {
  ...
  def generateHash(input: String): String = {
    try {
      if (input == null) {
        System.currentTimeMillis().toString+new Random().nextInt(1000000)
      }
